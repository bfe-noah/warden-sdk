...
- **`cru` — reset ladder.** Done. `CruSim` on `MemBus` (so `flared::devmem::hard_reset`'s
  ...
  one — lands when flare-edge consumes warden-sdk (§7 item 3, Noah-gated), not yet on
  flare-edge `main`.
...
which is a **Noah-go-ahead item** (credential/remote creation). Until then the
...
3. **flared devmem/hpmcu seam + tests** — firmware-side trait, unified with `sim/`
   once flare-edge consumes warden-sdk (a separate, Noah-gated step). **Pending.**
