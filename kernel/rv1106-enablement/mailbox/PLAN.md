...
### The `0x40000` boot-load hazard (boot-time MCU paths only)
...
1. **SCR1 echo firmware** (additive to our existing bare-metal firmware, must fit
   ...
   **proven `hpmcu.rs` load/release sequence** (SRAM path, `0xff6fe000`, **not**
   the `0x40000` boot path, hazard section 2).
...
---
_Cross-refs: `../CAPABILITIES-AUDIT.md`, `../REMAINING-PORTS.md section 7`,
