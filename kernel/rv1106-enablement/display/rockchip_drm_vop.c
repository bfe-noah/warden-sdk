...
struct vop {
	...
	bool is_enabled;
	/* flare-edge: U-Boot left the VOP scanning its boot logo and vop_initial
	 * kept it alive (skipped the resets, held clocks + a PM ref). Cleared,
	 * and the loader references dropped, on the first real vop_enable(). */
	bool loader_active;
...
static int vop_enable(struct drm_crtc *crtc, struct drm_crtc_state *old_state)
{
	...
	/* flare-edge: this enable now holds its own PM + clock references, so
	 * release the ones vop_initial() kept to preserve the boot logo. */
	if (vop->loader_active) {
		vop->loader_active = false;
...
static int vop_initial(struct vop *vop)
{
	...
	/*
	 * flare-edge: seamless takeover of a live bootloader scanout. If U-Boot
	 * left the VOP unstandby'd with a window enabled, it is actively
	 * scanning the boot logo (whose memory the bootloader protected via the
	 * rockchip,drm-logo reserved-memory fixup). Resetting the block here is
	 * what used to blank the panel seconds before userspace could draw, so
	 * in that case skip the AHB/dclk resets and the window disables, keep
	 * hclk/aclk/dclk and the runtime-PM reference held, and let the first
	 * real vop_enable() (the first modeset -- in practice the boot splash)
	 * take over and drop those references.
	 */
	{
		const struct vop_data *d = vop->data;
		u32 wins = 0;

		for (i = 0; i < d->win_size; i++)
			wins |= vop_read_reg(vop, d->win[i].base,
					     &d->win[i].phy->enable);
		vop->loader_active = wins &&
			!vop_read_reg(vop, 0, &d->common->standby);
	}
	...
	if (vop->loader_active) {
		/*
		 * Match the clk framework's book-keeping to the already-running
		 * hardware; these references (and the pm_runtime one from the
		 * top of this function) are dropped by the first vop_enable().
		 */
		ret = clk_enable(vop->dclk);
		if (ret < 0)
			DRM_DEV_ERROR(vop->dev, "loader takeover: dclk enable failed\n");
		DRM_DEV_INFO(vop->dev, "seamless takeover: bootloader scanout kept alive\n");
	} else {
		clk_disable(vop->hclk);
		clk_disable(vop->aclk);
	}
